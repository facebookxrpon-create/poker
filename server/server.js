...
io.on('connection', (socket) => {
  ...
  // Distribute Showdown Pots (Host Only + Auto 5s Countdown to Next Hand!)
  socket.on('distribute_pot', ({ roomCode, mainPotWinners, sidePotWinners }, callback) => {
    const room = rooms.get(roomCode?.toUpperCase());
    if (!room) return;

    const res = distributeShowdownPots(room, socket.id, { mainPotWinners, sidePotWinners });
    if (res.error) {
      if (typeof callback === 'function') callback({ success: false, error: res.error });
      return;
    }
    ...
    // Start 5-second interval timer for auto next hand
    if (room.countdownInterval) clearInterval(room.countdownInterval);

    room.countdownInterval = setInterval(() => {
      if (!room.hand || room.hand.countdown === undefined) {
        clearInterval(room.countdownInterval);
        room.countdownInterval = null;
        return;
      }

      room.hand.countdown -= 1;

      if (room.hand.countdown <= 0) {
        clearInterval(room.countdownInterval);
        room.countdownInterval = null;
        startNewHand(room);
        io.to(room.code).emit('room_updated', room);
