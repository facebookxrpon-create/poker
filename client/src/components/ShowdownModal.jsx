...
import { X, Trophy, Sparkles, Check, Crown, Clock } from 'lucide-react';
import confetti from 'canvas-confetti';
import { sounds } from '../utils/audio';

export default function ShowdownModal({ isOpen, onClose, room, socketId, onDistributePot }) {
  ...
  const handleSubmit = () => {
    if (!isHost) {
      setError('เฉพาะคนสร้างห้อง (Host) เท่านั้นที่เป็นคนกดแจกชิป');
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fadeIn">
      <div className="glass-panel w-full max-w-xl rounded-2xl border-2 border-amber-500/80 overflow-hidden shadow-2xl shadow-amber-950/80 max-h-[90vh] flex flex-col">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                {isHost ? 'คุณเป็น Host (คนสร้างห้อง) • เลือกผู้ชนะและกดแจกชิป' : 'รอ Host (คนสร้างห้อง) เป็นผู้เลือกผู้ชนะและแจกชิป'}
              </p>
            </div>
          </div>
          <button
        ...
        {/* Countdown Banner if hand is completed */}
        {hand.countdown > 0 && (
          <div className="bg-gradient-to-r from-emerald-600 via-emerald-500 to-emerald-600 p-3 text-center text-slate-950 font-black text-sm flex items-center justify-center gap-2 animate-pulse">
            <Clock className="w-5 h-5 animate-spin" />
            <span>เริ่มเล่น Hand ถัดไปอัตโนมัติใน {hand.countdown} วินาที...</span>
          </div>
        )}
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          {!isHost && (
            <div className="bg-amber-950/40 border border-amber-500/40 p-3 rounded-xl flex items-center gap-2 text-xs text-amber-300 font-semibold">
              <Crown className="w-4 h-4 text-amber-400 flex-shrink-0" />
              <span>เฉพาะคนสร้างห้อง (Host) เท่านั้นที่สามารถเลือกผู้ชนะและกด DISTRIBUTE POT ได้</span>
            </div>
          )}
          ...
          {hand.sidePots && hand.sidePots.map((sp, sideIdx) => {
            ...
            return (
              <div key={sideIdx} className="bg-slate-900/90 p-4 rounded-xl border border-emerald-500/40 space-y-3">
                ...
                <p className="text-xs text-slate-400">ผู้มีสิทธิ์ชนะ Side Pot นี้:</p>
          ...
          <div className="text-[11px] text-slate-400 bg-slate-900/60 p-3 rounded-xl border border-slate-800">
            💡 <strong>Odd Chip Rule:</strong> เศษชิป 1 หน่วยจากการหารไม่ลงตัว จะแจกให้ผู้ชนะที่อยู่ถัดไปทางซ้ายของตำแหน่ง Dealer
          </div>
        </div>
